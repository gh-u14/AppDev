...
# Скрипт для запуска проекта LR7 на Windows (Git Bash)

set -e

echo "Запуск проекта LR7..."
...
if ! docker-compose up -d postgres rabbitmq redis 2>/dev/null; then
    if ! docker compose up -d postgres rabbitmq redis 2>/dev/null; then
...
echo "  python app/consumer_app.py"
echo ""
